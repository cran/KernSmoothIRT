RelPlot<-function(OBJ, axis, quants, main, xlim, xlab, ylab, ...){
	

	axis <- axis[-1]
	
	inf<-IICplot(OBJ,axis=axis, quants=quants, xlab=xlab, test=TRUE,noout=TRUE)

	

	if(missing(main)){main<-"Reliability Plot\n"}
	if(missing(ylab)){ylab<-"Reliability"}
	if(missing(xlab)){xlab<-"Reliability"}
